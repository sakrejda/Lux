...
T_RET Slicer_Discrete<T_VAL, T_PMF, T_RET>::draw() {
	// for_each?
//	for (unsigned int i=0; i < (*plpmf).n_elem; ++i) CH.param(i,(*plpmf)[i]);
//	int ch = CH(*R);
	
// 1) Choose a value between 0 and lpmf at current x
	ly = *plpmf - EXPO(R);
	indexes.clear();
